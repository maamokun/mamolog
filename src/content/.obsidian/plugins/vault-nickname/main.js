...
var VaultNicknamePlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    try {
      ...
      console.error(
        "Could not determine if legacy settings file exists: " + legacySettingsFilePath,
      );
    }
    let migratedFromLegacySettings = false;
    if (legacySettingsExists && !sharedSettingsExists) {
      try {
        this.copyUtf8FileSync(legacySettingsFilePath, sharedSettingsFilePath);
        console.log(
          "Migrated a legacy shared settings file into the plugin's install directory: " +
    ...
    const needsLegacySettingsSaved =
      this.settings.enableBackwardsCompatibilty && !legacySettingsExists;
  ...
  onLayoutReady() {
    ...
    if (!originalDesktopVaultSwitcherElement) {
      console.error(
        "Vault switcher element not found. Cannot create element to intercept its events.",
      );
    } else {
      ...
      this.desktopVaultSwitcherElement.addEventListener(
        "click",
        this.onVaultSwitcherClicked.bind(this),
      );
      this.desktopVaultSwitcherElement.addEventListener(
        "contextmenu",
  ...
  onVaultSwitcherClicked(event2) {
    ...
    for (let vaultKey in vaults) {
      ...
      let vaultPluginSettingsFilePath = this.safeNormalizePath(
        [vaultPath, pluginInstallDir, VAULT_SHARED_SETTINGS_FILE_PATH].join(PATH_SEPARATOR),
      );
      ...
      if (!settingsFileExists) {
        vaultPluginSettingsFilePath = this.safeNormalizePath(
          [vaultPath, VAULT_LOCAL_LEGACY_SHARED_SETTINGS_FILE_PATH].join(PATH_SEPARATOR),
        );
  ...
  setAppTitle(vaultDisplayName) {
    ...
    if (this.settings.overrideAppTitle === "override-app-title:vault-first") {
      window.activeDocument.title = [vaultDisplayName, documentTitle, obsidianVersion].join(
        titleSeparator,
      );
    } else {
      window.activeDocument.title = [documentTitle, vaultDisplayName, obsidianVersion].join(
        titleSeparator,
      );
    }
  }
  ...
  getLegacySharedSettingsFilePath() {
    return this.safeNormalizePath(
      [this.app.vault.adapter.getBasePath(), VAULT_LOCAL_LEGACY_SHARED_SETTINGS_FILE_PATH].join(
        PATH_SEPARATOR,
      ),
    );
  }
